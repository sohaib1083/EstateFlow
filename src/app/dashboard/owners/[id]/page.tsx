  ...
  async function fetchOwnerProperties() {
    try {
      const { data, error } = await supabase
        ...
        `)
        .eq('property_owners.owner_id', params.id);
              ...
              <dl className="space-y-4">
                ...
                {owner.email && (
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">{owner.email}</dd>
                  </div>
                )}
                {owner.address && (
                  <div>
            ...
            <div className="p-6">
              {properties.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-gray-500 mb-4">No properties found for this owner.</p>
                  <Link
            ...
            <div className="mt-3">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <span className="ml-2 text-sm text-gray-900">{owner.full_name}</span>
                </div>
                <div>
                  <span className="text-sm font-medium text-gray-500">Phone:</span>
                  <span className="ml-2 text-sm text-gray-900">{owner.phone}</span>
                </div>
                {owner.email && (
                  <div>
                    <span className="text-sm font-medium text-gray-500">Email:</span>
                    <span className="ml-2 text-sm text-gray-900">{owner.email}</span>
                  </div>
                )}
                {owner.address && (
                  <div>
                    <span className="text-sm font-medium text-gray-500">Address:</span>
                    <span className="ml-2 text-sm text-gray-900">{owner.address}</span>
                  </div>
                )}
                <div>
